...
class WindowLevel(Dataset):
    ...
    def __len__(self):
        return len(self.windows)-9

    def __getitem__(self, index):
        windows = np.array(self.windows[index:index+9])
        try:
            label = self.labels[index+9]
        except IndexError:
            print("End of dataset reached, returning 0")
            return 0, 0
