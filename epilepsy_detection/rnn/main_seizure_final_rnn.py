...
dataset = WindowLevel(data_dir='../data/annotated_windows/', split_level='seizure')
print(f'LENGTH DATASET: {len(dataset)}')

# Prepare data for stratified k-fold
X = dataset.windows
y = dataset.labels
skf = GroupKFold(n_splits=5, shuffle=True, random_state=42)
...
torch.save(rnn_model.state_dict(), 'models/rnn_model_seizure.pth')
