...
# Open the video file
video_path = 'real_shadow.mp4'
cap = cv2.VideoCapture(video_path)
cap.set(cv2.CAP_PROP_FRAME_WIDTH, int(cap.get(cv2.CAP_PROP_FRAME_WIDTH) / 64))
cap.set(cv2.CAP_PROP_FRAME_HEIGHT, int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT) / 64))

# Write video
size = (450, 600) 
result = cv2.VideoWriter('shadow_b.avi',  
...
while cap.isOpened():
    ...
    if success:
        ...
        cv2.line(annotated_frame, (180,500), (400,500), (0,255,0), 2)
        ...
        for box, track_id, cls in zip(boxes, track_ids, classes):
            ...
            # Count
            if int(y) in [i for i in range(480,500)] and int(x) in [c for c in range(180,440)]:
