...
class Tracker:
        ...
        for rect in objects_rect:
            ...
            try:
                for id, ptl in self.center_points.items():
                    ...
                    if dist < 100 and len(self.center_points[id]) < 15:
                    ...
                    if len(self.center_points[id]) >= 15:
                        same_object_detected = True
    ...
    def self_update(self, id):
        ...
        if direction == "up":
            predicted_x = pred_final[0]
            predicted_y = max(pred_final[1], 223) 
        elif direction == "down":
            predicted_x = pred_final[0]
            predicted_y = min(pred_final[1], 509)
